...
export function BridgeConfigGeneratorClient() {
  ...
  const generateToken = async (opts?: { silent?: boolean }) => {
    try {
      ...
      if (!opts?.silent) toast.success(t("bridge.config.token_generated"));
    } catch (err: any) {
  ...
  // Auto-generate once when token is empty (per user+agentId), so users don't have to click "Generate" every time.
  useEffect(() => {
    if (!userId) return;
    if (tokenTouchedRef.current) return;
    if (token.trim()) return;
    ...
    const normalizedAgentId = agentId.trim();
    if (!normalizedAgentId) return;
    const guardKey = `${userId}:${normalizedAgentId}`;
    if (autoGeneratedRef.current === guardKey) return;

    // If we have a cached token, don't auto-generate.
    try {
      const cached = localStorage.getItem(tokenStorageKey(userId, normalizedAgentId));
      if (cached) {
        const parsed = JSON.parse(cached) as StoredAgentToken;
        if (parsed?.token && !isExpired(parsed.expires_at)) return;
      }
    } catch {
      // ignore
    }

    autoGeneratedRef.current = guardKey;
